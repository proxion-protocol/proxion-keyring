...
export const InstallationCenter = ({ proxionToken }) => {
    ...
    const fetchStatus = async () => {
        ...
        try {
            const resp = await fetch('http://localhost:8788/suite/status', {
    ...
    const handleAction = async (appId, action) => {
        ...
        try {
            const resp = await fetch(`http://localhost:8788/suite/${action}`, {
                method: 'POST',
                headers: {
            ...
            if (resp.ok) {
                fetchStatus();
                // Trigger Homarr Tutorial if this was an install of a main app
                if (action === 'install') {
                    const onboardingSeen = localStorage.getItem('homarr_tutorial_seen');
                    if (!onboardingSeen) {
                        // Signal parent to show tutorial
        ...
        <div className="installation-center">
            ...
            <div className="app-grid">
                {filteredApps.map(app => {
                    const appStatus = status[app.id] || 'UNINSTALLED';
                    const isWorking = installingId === app.id;
                    const baseSlug = app.logo_slug || app.id.replace('-integration', '');
                    const simpleIconUrl = app.logo_slug ? `https://cdn.simpleicons.org/${app.logo_slug}?viewbox=auto` : null;
                    const dashboardIconUrl = `https://cdn.jsdelivr.net/gh/walkxcode/dashboard-icons/png/${baseSlug}.png`;
                        ...
                        <div className={`app-card-modern ${appStatus.toLowerCase()}`} key={app.id}>
                            <div className="card-top">
                                <div className="logo-wrapper">
                                    <img
                                        src={simpleIconUrl || dashboardIconUrl}
                                        alt={app.name}
                                        className="app-logo"
                                        crossOrigin="anonymous"
                                        loading="lazy"
                                        onError={(e) => {
                                            // Fallback chain: SimpleIcon -> DashboardIcon -> Hidden (show css icon)
                                            if (e.target.src === simpleIconUrl && dashboardIconUrl) {
                                                e.target.src = dashboardIconUrl;
                                            } else {
                                                e.target.style.display = 'none';
                                                const fallback = e.target.parentElement.querySelector('.app-icon-backup');
                                ...
                                <div className="status-badge">
                                    ...
                                    <span>{appStatus === 'UNINSTALLED' ? 'Available' : appStatus}</span>
                                </div>
                            </div>
                                ...
                                {appStatus === 'UNINSTALLED' ? (
                                    ...
                                    <div className="action-row">
                                        ...
                                        <button
                                            ...
                                            onClick={() => window.open(`http://localhost:${app.port || 80}`, '_blank')}
                                        >
                                            Open UI
                                        </button>
                                        {appStatus !== 'RUNNING' && (
            ...
            <style>{`
                .filter-bar {
                    ...
                    margin-bottom: 2rem;
                    padding: 0.5rem 0;
                    /* Removed overflow to allow wrapping */
                }
